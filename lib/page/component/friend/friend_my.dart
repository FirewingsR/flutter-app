...
class FriendMy extends StatefulWidget {
  FriendMy({Key key, this.id}) : super(key: key);

  final String id;

  @override
  _FriendMyState createState() => _FriendMyState(id);
}

class _FriendMyState extends State<FriendMy> {
  List<User> _list = [];
  ...
  _init(id) async {
    if (id == "0" || id == null) {
      id = await LocalStorage().get("userId");
    }
    _id = id;
    _initData();
  }
